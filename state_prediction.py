...
def Quantum_map(Nreal, Detection, args, fig_qm=True):
    ...
    M_list = []
    for NH in args['NHs']:
         M_list+= [qt.Qobj(np.zeros((NH,NH)))]
    Null_matrix = qt.tensor(M_list)
...
def RhoFinal(Nrealmax, Detection, args, fig_rhofinal=True, fig_qm=False):
    ...
    if 1.0 - Rho_final.tr() > 0.01:
        print ("\n The trace of the density matrix is "+str(np.float(int(round(Rho_final.tr()*100))/100)))
